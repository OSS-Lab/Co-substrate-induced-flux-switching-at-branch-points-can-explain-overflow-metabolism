...
def make_data_parallel(fn,inp_args):#,totleng):
    ...
    threads the total
    # """
    with multi.Pool(os.cpu_count()-6) as pool:
...
def get_fluxfractions_concs(lengthparams,B_position_params,
            params_global,params_B,
            params_branch,
            params_inout,
            reversible,Tmax,tpts):
...
def return_m2_b0(lengthparams=[2,1,1],B_position_params=([1,0],[0,0],[1],[1]),
            params_global=[1.,1.,1.,1.],params_B=[0.1,1.,1.,1.,10.],
            params_branch=[1.,1.,1.,1.,
                           1.,1.,1.,1.],
            kouts=[1.,1.],#params_inout=[0.1,1.,1.],
            kinvals=np.logspace(-4,0,21),
   ...
   # print(kinvals)
    ...
    B0conc=np.zeros(np.size(kinvals))
    B0conc[:]=np.nan
    
    M2conc=np.zeros(np.size(kinvals))
    M2conc[:]=np.nan
    for id_kin, kin in enumerate(kinvals):
        params_inout=np.concatenate(([kin],kouts))
   #     print(np.log10(kin))
        concs =get_fluxfractions_concs(lengthparams,B_position_params,
            params_global,params_B,params_branch,
            params_inout,
            reversible,Tmax,tpts)
   #     print(isbuildup)
      #  print(concs)
        B0conc[id_kin]=concs[-2]
        M2conc[id_kin]=concs[2]
   # print(B0conc)
    #print(M2conc)
    return B0conc,M2conc
...
def heatmap_makedata_test():
    ...
    # params_branch unchanging
    Ks_top=1. #
    Kp_top=1.
    Ks_bottom=1.
    Kp_bottom=1.

    # params_inout unchanging
    kout_top=1.
    kout_bottom=1.

    # params_b variable
    CKcatB_vals=np.logspace(-3,3,6)    ### xvars
    Keqb_vals=np.logspace(-2,0,6)     ###### loop over figures
    Btot_vals=np.logspace(-3,3,6)      ### yvars

    # params_branch variable
    CKcat_top_vals=[1.]#np.logspace(-2,0,2)
    Keq_top_vals=[1.]#np.logspace(-2,0,2)
    CKcat_bottom_vals=np.logspace(-3,3,6)   #####  loop over rows
    Keq_bottom_vals=[1.]#np.logspace(-2,0,2)

    # params_inout variable
    kin_min=-8
    kin_max=0
    num_kin=41
    ...
    t0=time.time()
    # list_of_args=Gen_all_data()
    list_of_args=Gen_all_data(lengthparams,B_position_params,
                              CKcat,Ks,Kp,Keq,                      # params_global
                              KsB,KpB,                              # params_b unchanging
                              Ks_top,Kp_top,Ks_bottom,Kp_bottom,    # params_branch unchanging
                              kout_top,kout_bottom,                 # params_inout unchanging
                              CKcatB_vals,
                              Keqb_vals,
                              Btot_vals,                            # params_b variable
                              CKcat_top_vals,
                              Keq_top_vals,
                              CKcat_bottom_vals,
                              Keq_bottom_vals,                      # params_branch variable
                              kin_min,
                              kin_max,
                              num_kin,
                              reversible,
  ...
  #  print(list_of_args)
    ...
    if len(list_of_args)>0:
        make_data_parallel(get_ff_delta,list_of_args)
    t1=time.time()
...
#heatmap_makedata_test()


def make_heatmaps(xvar='Btot',xvarvals=np.logspace(-3,3,25),
