...
plt.axis('off')
motifnone=img.imread('figS7_mot_nochange.png')
plt.imshow(motifnone)
ax2=plt.axes([0.7,0.6,0.25,0.3])
ax2.text(10**-17.3,10**2.,'$\\rm A$', fontsize=35)
ax2.text(10**-3,10**1.7,'$\\rm B$', fontsize=35)

ax5=plt.axes([1.05,0.6,0.3,0.3])
...
cax = ax2.inset_axes([1.05, 0, 0.04, 0.85])
cbar=plt.colorbar(im, cax=cax, orientation='vertical',ticks=[-1, 0, 1])
cbar.ax.tick_params(labelsize=20)
ax2.text(10**3.2,10**2.5,'$\\Delta F_f$',fontsize=25,rotation=0)

# cbar = plt.colorbar(im)
# #cbar.set_label('$\\Delta F_f$',fontsize=30,rotation=0,labelpad=-40, y=1.15)
# cbar.set_label('$\\Delta F_f$',fontsize=30,rotation=0,labelpad=0, y=0.92)

V24_vals=np.array([0.1,1,10])
    
cols=['blue','black','red']
inset1=plt.axes([1.235,0.787,0.1,0.1])
...
for j in range(len(V24_vals)):
    ...
    ax5.plot(kinvals,ff_vec,color=cols[j],label='$V_{\\rm max}^{24} = '+str(V24)+'$')
    inset1.plot(kinvals,b_vec,color=cols[j])#,linestyle=':') # add as inset
    
ax5.set_xscale('log')
# ax5.set_yscale('log')
ax5.legend(loc=(0.59,0.05),fontsize='large')
ax5.set_ylim(0,1)
ax5.set_xticks(np.logspace(-4,0,5),['$10^{-4}$','','$10^{-2}$','','$10^0$'])
ax5.set_yticks(np.linspace(0,1,5),['$0$','','$0.5$','','$1$'])
ax5.set_xlabel('$v_{\\rm in}$',fontsize=25)
ax5.xaxis.set_label_coords(0.7,-0.05)
ax5.set_ylabel('$F_f$',fontsize=25,rotation=0)
ax5.yaxis.set_label_coords(-0.1,0.6)
ax5.text(10**-4,0.75,'$\\rm C$', fontsize=35)


inset1.set_xscale('log')
# inset1.set_yscale('log')
inset1.set_ylim(0,1)
inset1.set_xticks(np.logspace(-4,0,3),['$10^{-4}$','','$10^0$'],fontsize=15)
inset1.set_yticks(np.linspace(0,1,3),['$0$','','$1$'],fontsize=15)
