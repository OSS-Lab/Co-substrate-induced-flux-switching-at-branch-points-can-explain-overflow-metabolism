...
# -*- coding: utf-8 -*-
"""
Created on Mon May 12 12:20:05 2025
...
ax2=plt.axes([1,-0.6,0.9,1.1])    # scatter
...
ax6=plt.axes([3.35,0.7,0.35,0.35])
ax7=plt.axes([3.35,1.05,0.35,0.35])
...
ax2.text(-0.15,1.85,'$\\rm A$',fontsize=40)
ax2.text(-0.15,0.85,'$\\rm C$',fontsize=40)
ax2.text(1.15,0.9,'$\\rm D$',fontsize=40,color='limegreen')
ax2.text(2.5,0.85,'$\\rm E$',fontsize=40,color='royalblue')
ax2.text(1.8,1.85,'$\\rm B$',fontsize=40)
...
for i in range(n):
    ...
    elif ff_delta[i]<delwant_noswitch:
...
ax2.scatter(ff_delta,c0delta,s=40,color='bisque',alpha=0.5,edgecolors='k',zorder=0)#,label='')
ax2.scatter(ff_delta[indplot_switchretained],c0delta[indplot_switchretained],s=40,color='limegreen',alpha=1,edgecolors='k',zorder=20)#,label='')
ax2.scatter(ff_delta[indplot_noncosub],c0delta[indplot_noncosub],s=40,color='royalblue',alpha=1,edgecolors='k',zorder=20)#,label='')
...
ax2.yaxis.set_label_coords(-0.09,0.63)
...
ax3.text(10**-7,0.6,'NADH \n (mito)', fontsize=20, color='mediumseagreen')
ax3.text(10**-6,0.25,'NADH \n (cyto)', fontsize=20, color='peru')
ax3.text(10**-7,0.25,'NADH \n (total)', fontsize=20, color='firebrick')
ax3.text(10**-6,0.6,'$F_f$', fontsize=20, color='royalblue')


ax3.set_xlabel('$v_{\\rm in}$',fontsize=30)
ax3.set_ylabel('$ \\rm Chemical ~Fraction$',fontsize=30)
# ax3.legend(loc='upper left',ncols=1)
ax3.set_xscale('log')

ax3.set_xticks(np.logspace(-9,-2,8))#, ['$10^{-9}$','','$10^{-7}$','',
                                    # '$10^{-5}$','','$10^{-3}$',''])
...
ax4.set_xticks(np.logspace(-9,-2,8))#, ['$10^{-9}$','','$10^{-7}$','',
                                     #'$10^{-5}$','','$10^{-3}$',''])
...
ax5.plot(xvals_dilution_rate,yvals_eth_prod_rate,'-')
ax5.plot(xvals_dilution_rate,yvals_eth_prod_rate,'x',color='k',
         label='Ethanol Production Rate')
ax5.set_xlabel('${\\rm Dilution ~Rate}~(h^{-1})$',fontsize=20)
...
sns.barplot(x=xvals, y=yvals_NADplus, ax=ax6,color='gold')
# plt.xticks(rotation=45)
ax6.set_yticks(np.linspace(0,150000,4))
ax6.set_yticklabels(['$0$','','$10^5$',''],fontsize=15)
ax6.set_xticklabels(['$0.06$','$0.11$','$0.16$','$0.24$','$0.31$'],
                 fontsize=13,rotation=45)
ax6.set_xlabel('${\\rm Dilution ~Rate}~(h^{-1})$',fontsize=20)
ax6.set_ylabel('${\\rm Normalised~Ion~counts}$',fontsize=20)
ax6.yaxis.set_label_coords(-0.2,0.8)
ax6.text(0.6,1.25*10**5,'$\\rm{ NAD}+$',fontsize=20)

sns.barplot(x=xvals, y=yvals_NADH, ax=ax7)
ax7.set_yticks(np.linspace(0,6000,7))
ax7.set_yticklabels(['$0$','','','','','$5 \\cdot 10^3$',''],fontsize=15)
ax7.set_xticks([])
ax7.text(0.6,4*10**3,'$\\rm{ NADH}$',fontsize=20)


dilRate=[	0.05	,0.11	,0.16	,0.22	,0.3]
NAD=[	0.001171528	,0.001096782	, 0.001090527,	0.001043792,	0.000807479]
NADH	=[3.93345E-06	,4.55817E-06	,5.18715E-06	,8.31691E-06	,1.97706E-05]

sns.barplot(x=dilRate, y=NAD, ax=ax8,color='gold')

ax8.set_yticks(np.linspace(0,0.0015,4))
# ax8.set_yticklabels(['$0$','$0.0005$','$0.001$',''],fontsize=15)
ax8.set_yticklabels(['$0$','','$10^{-3}$',''],fontsize=15)
ax8.set_xticklabels(['$0.05$','$0.11$','$0.16$','$0.22$','$0.3$'],
                 fontsize=13,rotation=45)
ax8.set_xlabel('${\\rm Dilution ~Rate}~(h^{-1})$',fontsize=20)
ax8.set_ylabel('${\\rm Asbolute~Concentration}$',fontsize=20)
ax8.yaxis.set_label_coords(-0.25,0.8)

sns.barplot(x=dilRate, y=NADH, ax=ax9)
ax9.set_yticks(np.linspace(0,2*10**-5,3))
ax9.set_yticklabels(['$0$','$10^{-5}$','$2\\cdot 10^{-5}$'],fontsize=15)
ax9.set_xticks([])
# ax9.text(0.6,4*10**3,'$\\rm{ NADH}$',fontsize=20)
