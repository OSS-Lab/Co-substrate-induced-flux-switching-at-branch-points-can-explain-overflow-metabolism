...
sys.path.append('../simulation_functions_nocosub')
...
for al in aldo:
	...
	args_to_run=mh_cm.make_params_newtest_5_nonfixed(allo,n)

	now=dt.datetime.now()
	print(now)

	savepath='../data/allo_'+str(allo)
	filename='/nocosub_nonfixed_apr_final_seed'+seedstr+'_n_'+str(n)+'_datetime'+str(now)
	...
	np.savez(savename, flux_cyto=ftlist, flux_mito=fblist, flux_diffn=fdlist,
			 ...
			 c0delta=c0deltalist, n0delta=n0deltalist)
...
for al in aldo:
	...
	args_to_run=mh_cm.make_params_newtest_5(allo,n)

	now=dt.datetime.now()
	print(now)

	savepath='../data/allo_'+str(allo)
	filename='/nocosub_fixed_apr_final_seed'+seedstr+'_n_'+str(n)+'_datetime'+str(now)
	savename=savepath+filename
