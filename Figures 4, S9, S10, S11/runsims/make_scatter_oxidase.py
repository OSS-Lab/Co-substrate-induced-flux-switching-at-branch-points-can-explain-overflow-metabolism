...
for al in aldo:
    for modval in mods:
        ...
        args_to_run=mh_cm.make_params_newtest_6_nonfixed(allo,n,modval[0],modval[1],modval[2],modval[3])
    
        if modval[-1]<1:
            filename = '/nonfixed_apr_final_KCmod_seed'+seedstr+'_n_'+str(n)+'_datetime'+str(now)
        else:
            filename = '/nonfixed_apr_final_KBmod_seed'+seedstr+'_n_'+str(n)+'_datetime'+str(now)
        ...
        np.savez(savename, flux_cyto=ftlist, flux_mito=fblist, flux_diffn=fdlist,
                 ...
                 c0delta=c0deltalist, n0delta=n0deltalist, swpt=swptlist)
