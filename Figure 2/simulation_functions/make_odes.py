...
def make_ode_vectors(lengthparams=[3,2,2],B_position_params=([0,1,0],[0,0,0],[1,0],[1,0]),
              params_global=[1.,1.,1.,1.],params_B=[0.1,1.,1.,1.,0.1],allo=[1.,1.,1.,1.],
...
def make_ode_func(lengthparams=[3,2,2],B_position_params=([0,1,0],[0,0,0],[1,0],[1,0]),
    ...
    params=np.concatenate((params_inout, params_global, params_branch,params_B,allo))
    # kin =             params[0]
    # kouttop =         params[1]
    # koutbottom =      params[2]
...
def get_branch_fluxes(result,odefunc,params,lengthparams,B_position_params,reversible):
    ...
    if reversible == False:
        if topbranch10[0]==0:
            flux_top=reac.mu(Mbranch_final**n23, Mtop_final, 1., 1., CKcat_top, Ks_top**n23, Kp_top, Keq_top)
        else:
            flux_top=reac.mu(Mbranch_final**n23, Mtop_final,B1_final**m23, B0_final, CKcat_top, Ks_top**(n23+m23), Kp_top, Keq_top)
            
        if bottombranch01[0]==0:
            flux_bottom=reac.mu(Mbranch_final**n24, Mbottom_final, 1., 1., CKcat_bottom, Ks_bottom**n24, Kp_bottom, Keq_bottom)
        else:
            flux_bottom=reac.mu(Mbranch_final**n24, Mbottom_final, B0_final**m24, B1_final, CKcat_bottom, Ks_bottom**(n24+m24), Kp_bottom, Keq_bottom)
    else:
        if topbranch10[0]==0:
            flux_top=reac.forwardflux_rev(Mbranch_final**n23, Mtop_final, 1., 1., CKcat_top, Ks_top**n23, Kp_top, Keq_top)
        else:
            flux_top=reac.forwardflux_rev(Mbranch_final**n23, Mtop_final,B1_final**m23, B0_final, CKcat_top, Ks_top**(n23+m23), Kp_top, Keq_top)
            
        if bottombranch01[0]==0:
            flux_bottom=reac.forwardflux_rev(Mbranch_final**n24, Mbottom_final, 1., 1., CKcat_bottom, Ks_bottom**n24, Kp_bottom, Keq_bottom)
        else:
            flux_bottom=reac.forwardflux_rev(Mbranch_final**n24, Mbottom_final, B0_final**m24, B1_final, CKcat_bottom, Ks_bottom**(n24+m24), Kp_bottom, Keq_bottom)
