...
def find_sw_pt_new(kinplot,fluxfrac,fluxfrac_mod,delta=False):
    
    
    ###### FIND PT AND VAL FOR ORIG ##################
    max_orig=np.nanmax(fluxfrac)
    min_orig=np.nanmin(fluxfrac)
    sw_orig=(max_orig+min_orig)/2.### half way switch point value for orig
    
    arg_sw=np.argmax(fluxfrac>sw_orig) ####index for kinvalue 
    
    kin_orig_min=kinplot[arg_sw-1]
    kin_orig_max=kinplot[arg_sw]
    swpt_orig=(kin_orig_min+kin_orig_max)/2.
    
    sw_min=fluxfrac[arg_sw-1]
    sw_max=fluxfrac[arg_sw]
    swval_orig=(sw_min+sw_max)/2.
    
    ###### FIND PT AND VAL FOR MOD #############################
    
    max_mod=np.nanmax(fluxfrac_mod)
    min_mod=np.nanmin(fluxfrac_mod)
    sw_mod=(max_mod+min_mod)/2.
    
    arg_sw_mod=np.argmax(fluxfrac_mod>sw_mod) ####index for kinvalue 
    
    kin_mod_min=kinplot[arg_sw_mod-1]
    kin_mod_max=kinplot[arg_sw_mod]
    swpt_mod=(kin_mod_min+kin_mod_max)/2.
    
    sw_min_mod=fluxfrac_mod[arg_sw_mod-1]
    sw_max_mod=fluxfrac_mod[arg_sw_mod]
    swval_mod=(sw_min_mod+sw_max_mod)/2.
    
    ###### FIND VAL FOR ORIG WITH MOD PT
    
    arg_sw_check=np.argmax(fluxfrac>sw_mod)
    # print(fluxfrac>sw_mod)
    # print(arg_sw_check)
    kin_orig_min_check=kinplot[arg_sw_check-1]
    kin_orig_max_check=kinplot[arg_sw_check]
    # print(kin_orig_min_check,kin_orig_max_check)
    swpt_orig_check=(kin_orig_min_check+kin_orig_max_check)/2.
    
    sw_min_check=fluxfrac[arg_sw_check-1]
    sw_max_check=fluxfrac[arg_sw_check]
    swval_orig_check=(sw_min_check+sw_max_check)/2.
    
    ###### FIND VAL FOR MOD WITH ORIG PT
    
    arg_sw_mod_check=np.argmax(fluxfrac_mod>sw_orig)
    # print(fluxfrac_mod>sw_orig)
    # print(arg_sw_mod_check)
    kin_mod_min_check=kinplot[arg_sw_mod_check-1]
    kin_mod_max_check=kinplot[arg_sw_mod_check]
    # print(kin_mod_min_check,kin_mod_max_check)
    swpt_mod_check=(kin_mod_min_check+kin_mod_max_check)/2.
    
    sw_min_mod_check=fluxfrac[arg_sw_mod_check-1]
    sw_max_mod_check=fluxfrac[arg_sw_mod_check]
    swval_mod_check=(sw_min_mod_check+sw_max_mod_check)/2.

    # print('####')
    sw_delta=100*(swpt_mod-swpt_orig)/swpt_orig
    if delta==True:
        # print(swpt_orig,swpt_mod_check)
        # print(swpt_mod,swpt_orig_check)
        # print(np.abs(swpt_orig-swpt_mod_check))
        # print(np.abs(swpt_mod-swpt_orig_check))
        # print(min(np.abs(swpt_orig-swpt_mod_check),np.abs(swpt_mod-swpt_orig_check)))
        
        if min(np.abs(swpt_orig-swpt_mod_check),np.abs(swpt_mod-swpt_orig_check))<0.001:
            # print('need to filter these out')
            sw_delta=np.nan
            
        # elif 
        # if np.abs()
    # print(sw_orig,sw_mod)
        
    
    return swpt_orig,swval_orig,swpt_mod,swval_mod,swpt_orig_check,swval_orig_check,swpt_mod_check,swval_mod_check,sw_delta
...
# kin_test=np.logspace(-4,0,41)
# ff_orig_test=kin_test**2./(kin_test**2.+10**-4)
# ff_mod_test=kin_test**2./(kin_test**2.+10**-4)
# ff_mod_test[-30:]=np.nan
# plt.figure()
# plt.plot(kin_test,ff_orig_test,'b')
# plt.plot(kin_test,ff_mod_test,'r')
# plt.xscale('log')

# swpt_orig,swval_orig,swpt_mod,swval_mod,swpt_orig_check,swval_orig_check, swpt_mod_check,swval_mod_check,sw_delta = find_sw_pt_new(kin_test,ff_orig_test,ff_mod_test,delta=True)

# # print(swpt_orig,swval_orig)
# plt.scatter(swpt_orig,swval_orig,50,'b')
# plt.scatter(swpt_mod,swval_mod,50,'r')

# plt.scatter(swpt_orig_check,swval_orig_check,10,'c')
# plt.scatter(swpt_mod_check,swval_mod_check,10,'g')
...
def make_params_nov25_allranges_nonfixed(allo,n,VBmod=1.,VCmod=1.,KBmod=1.,KCmod=1.,diffval=1.,diffscale=2.):#,largercosubtot=False):
   ...
   # print(fixed,fasttrans,fastdiff,smallercosubs,fasterbackground,asymdiff)
    for i in range(n):
      ...
      #  print(ksvals)
        ...
        ########  do between 0.1, 1 rather than 0.1 and 10
        randvals=10**np.random.uniform(-1,1,40)
        randvals_tots=10**np.random.uniform(-1,0,2)
        
        
        Btot=3*randvals_tots[0]
        Ctot=0.3*randvals_tots[1]
   

        # print(Btot,Ctot)
        params01=[4.4*randvals[0]/1000.,(0.21*randvals[1])*(0.09*randvals[2]),
                  (9.8*randvals[3]*10**-3)*(0.06*randvals[4]),0.2]  #GAP->BPG
        
        params1E=[5.18*randvals[5]/1000.,randvals[6]/10.,randvals[7]/10.,
                  4074.]    #BPG->PEP
        
        paramsE2=[4.05*randvals[8]/1000.,(0.14*randvals[9])*(0.53*randvals[10]),
                  (21*randvals[11])*(1.5*randvals[12]),6.8*10**4]   #PEP->PYR
        
        params35=[3*10*randvals[13]/1000.,(1.1*randvals[14])*(0.11*randvals[15]),
                  (17.*randvals[16])*(0.17*randvals[17]),5.*10**3]
 
        params_35=[4.3*randvals[18]/1000.,(0.44*randvals[19])*(0.07*randvals[20]),
                   (12.*randvals[21])*(0.24*randvals[22]),5.*10**3]
        
        paramsB=[(0.6*randvals[23]/1000.)*VBmod ,randvals[24],randvals[25]/10.,0.1*KBmod]
        paramsC=[(0.4*randvals[26]/1000.)*VCmod ,randvals[27]*10.,randvals[28]/100.,0.01*KCmod]
        
        
        
        params23=[0.65*randvals[29]/1000.,4.33*randvals[30]]#,5.,2.4*10**2]
        
        params_trans=[0.02*randvals[31]/1000.,0.64*randvals[32]]# vt kt
        
        params_24=[0.8*randvals[33]/1000.,0.5*randvals[34]] 
        # dummy=10**rn.uniform(-4,0)
        # param_diff2=10**rn.uniform(-4,0)
        
        
        params_diff_5=[diffval*randvals[35],diffscale*diffval*randvals[35]]
        params_diff_3=[diffval*randvals[37],diffscale*diffval*randvals[37]]
...
def make_params_nov25_allranges_fixed(allo,n,VBmod=1.,VCmod=1.,KBmod=1.,KCmod=1.,diffval=1.,diffscale=2.):#,largercosubtot=False):
   ...
   # print(fixed,fasttrans,fastdiff,smallercosubs,fasterbackground,asymdiff)
    for i in range(n):
      ...
      #  print(ksvals)
        ...
        params_trans=params23#[0.02*randvals[31]/1000.,0.64*randvals[32]]# vt kt
        
        params_24=params23#[0.8*randvals[33]/1000.,0.5*randvals[34]] 
        # dummy=10**rn.uniform(-4,0)
        # param_diff2=10**rn.uniform(-4,0)
