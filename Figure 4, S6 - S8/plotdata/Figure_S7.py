...
ax15.text(-1.37,12.5,'$\\rm A$',fontsize=40)
ax15.text(-0.15,12.5,'$\\rm B$',fontsize=40)
...
filename='nonfixed_apr_final_seed'+seedstr+'_n_'+str(n)+'_'
...
# ax14.yaxis.set_label_coords(-0.2,0.5)

ax17.scatter(ff_delta,maxdelta,s=10,color='cornflowerblue',alpha=scat_alpha )
ax17.axis([-0.05,1.05,-0.05,1.05])
ax17.set_xlabel('$\\Delta F_f$',fontsize=20)
ax17.set_ylabel('$\\max{\\lbrace\\Delta N_{\\rm H}^{m},\\Delta N_{\\rm H}^{c}\\rbrace}$',
                fontsize=20,rotation=90)
ax17.yaxis.set_label_coords(-0.2,0.5)
ax17.xaxis.set_label_coords(0.7,-0.1)
   

filename='nocosub_nonfixed_apr_final_seed'+seedstr+'_n_'+str(n)+'_'
savename=savepath+filename
print(savepath)
print(filename)
p=Path(savepath)
...
# ax14.set_xlabel('flux frac delta')
ax14.legend(loc=(0.2,0.3),title='\\underline{Parameters as in Table 1}')
ax14.axis([-0.05,1.05,0,4])
ax14.set_yticks(np.linspace(0,4,5))
...
filename='fixed_apr_final_seed'+seedstr+'_n_'+str(n)+'_'
savename=savepath+filename
print(savepath)
print(filename)
p=Path(savepath)
...
ax15.yaxis.set_label_coords(-0.05,0.5)

ax16.scatter(ff_delta,maxdelta,s=10,color='mediumseagreen',alpha=scat_alpha)
ax16.axis([-0.05,1.05,-0.05,1.05])
ax16.set_xlabel('$\\Delta F_f$',fontsize=20)
ax16.set_ylabel('$\\max{\\lbrace\\Delta N_{\\rm H}^{m},\\Delta N_{\\rm H}^{c}\\rbrace}$',
                fontsize=20,rotation=90)
ax16.yaxis.set_label_coords(-0.2,0.5 )
ax16.xaxis.set_label_coords(0.7,-0.1)

filename='nocosub_fixed_apr_final_seed'+seedstr+'_n_'+str(n)+'_'
savename=savepath+filename
print(savepath)
print(filename)
p=Path(savepath)
...
ax15.legend(loc=(0.4 ,0.35),title='\\underline{Branch-point enzyme parameters set equal}',ncols=2)

if savefig==1:
    plt.savefig('Figure_S7.png',bbox_inches='tight',format='png')
    plt.savefig('Figure_S7.eps',bbox_inches='tight',format='eps')
    plt.savefig('Figure_S7.pdf',bbox_inches='tight',format='pdf')
    plt.savefig('Figure_S7.svg',bbox_inches='tight',format='svg')
